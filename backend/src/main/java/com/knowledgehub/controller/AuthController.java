...
public class AuthController {
    ...
    /**
     * Refresh endpoint: client sends the refreshToken in the JSON body.
     * No cookies involved — works correctly cross-domain (Vercel -> Render).
     */
    @PostMapping("/refresh")
    public ResponseEntity<ApiResponse<AuthResponse>> refresh(
        ...
        if (refreshToken == null || refreshToken.isBlank()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(ApiResponse.error("No refresh token provided. Please log in again.", 401));
        }
    ...
    public ResponseEntity<ApiResponse<Void>> logout() {
        // Client is responsible for clearing its own localStorage.
